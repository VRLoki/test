...
if __name__ == "__main__":
    ...
    col = client.tmp_db_6.tmp_col_6
    
    # created 
    obj_ids_matched = []
    col.insert_one({ 'name': "doc 0", 'topics': ["topic_0"]})
    col.insert_one({ 'name': "doc 1", 'topics': ["topic_1"]})
    col.insert_one({ 'name': "doc 2", 'topics': ["topic_2"]})
    col.insert_one({ 'name': "doc 3", 'topics': ["topic_3"]})
    col.insert_one({ 'name': "doc 4", 'topics': ["topic_4"]})

    # search
    items = schools_by_topic(col, "topic_10")
    for item in items:
