...
int trace_pid(pid_t tracee, bool_t is_child, char **av, char **env)
{
	...
	while (TRUE)
	{
		...
		if (strcmp(syscall->name, "read") != 0 &&
			strcmp(syscall->name, "fstat") != 0 &&
			!print_special(tracee, &regs, syscall))
		...
		if (strcmp(syscall->name, "read") == 0)
			print_read_args(tracee, &regs, syscall);
		if (strcmp(syscall->name, "fstat") == 0)
			print_fstat_args(tracee, &regs, syscall);
